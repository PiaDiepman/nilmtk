...
LIMIT = 100000          # Limit to shorten runtime for testing


start_date = datetime(2015, 1, 29, 0, 0, 0+00)
end_date = datetime(2015, 2, 5, 23, 59, 59+00)

def convert_dataport_dates(input_path, hdf_filename,
    ...
    ----------
    input_path : str
        The root path of the Pecan Dataport dataset.
    hdf_filename : str
    ...
    for file in files:
        ...
        data_iter = pd.read_csv(file_path, header=0, chunksize=1, parse_dates=True)   #, index_col=['localminute']
        # Iterate through each row of the dataframe
        for row in data_iter:
            # Load the csv as an iterator over a dataframe
            file_path = input_path + "/" + file
            data_iter = pd.read_csv(file_path, header=0, chunksize=1)  #, nrows=LIMIT
            fields = next(data_iter)
            print(fields)
            # Iterate through each row of the dataframe
            for row in data_iter:
               # if row["localminute"].values[0] >= start_date and row["localminute"].values[0] < end_date: 
                # Get dataid - it is different for each household
                dataid = row["dataid"].values[0]
                minutetime = row["localminute"].values[0]
                formatted_date = datetime.strptime(minutetime, "%Y-%m-%d %H:%M:%S+00")
               # print(start_date, end_date, formatted_date)
                if formatted_date >= start_date and formatted_date <= end_date: 
                    # Locate csv path
                    path_csv = build_tmp_dir + "/" + str(dataid) + ".csv"
                    #If a file for the building id does noit exist, create one including headings
                    if not os.path.isfile(path_csv):
                       row.to_csv(path_csv, header=True)
                    else: # else a fiel for the building id already so append new data line without writing the header
                       row.to_csv(path_csv, mode='a', header=False)
    ...
    for file in buildings:
        ...
        print("Loading building {:d} and nilmtk number {} @ {}".format(building_id, nilmtk_building_id, datetime.datetime.now()))
        sys.stdout.flush()
        dataframe = pd.read_csv(build_tmp_dir + "/" + file)  #index_col=0
        # Convert date column from string to date format
        dataframe[time_column] = pd.to_datetime(dataframe[time_column], utc=True )  # utc=True - not needed for 1 day
